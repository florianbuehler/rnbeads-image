...
# 				path = "/data/samplesheet.csv" #<- worked somehow
				# update the widget value
...
observeFakeLocalFileInput <- function(input, session, inputId, path){
	observeEvent(ignoreNULL=TRUE, eventExpr={input[[inputId]]},
		handlerExpr={
			if (input[[inputId]] > 0) {
				updateLocalFileInput(session, inputId, value=path)
...
ui <- tagList(useShinyjs(), navbarPage(
	...
	tabPanel(tags$div(title="Specify general parameters, such as report (output) directories, number of cores to use, etc. You can configure new analyses or inspect the status of previous RnBeads runs.", icon("bar-chart"), "Analysis"),
		sidebarPanel(
			tags$div(title="Specify parent directory where the analysis reports will be stored.", directoryInput('outDir', label = 'Select analysis directory', value = '/reports')),
...
# 			tags$div(title="Select a file which contains a valid annotation table with rows corresponding to each sample in the analysis.", textInput("sampleAnnotFile", "Select sample annotation file", "/data/samplesheet.csv")),
...
# 			tags$div(title="Specify the directory where the input methylation data (IDAT or methylation call files) can be found.", directoryInput('dataDir', label='Select input data directory'))
			tags$div(title="Specify the directory where the input methylation data (IDAT or methylation call files) can be found.", directoryInput('dataDir', label='Select input data directory', value = "/data/raw-data"))
		),
		mainPanel(
...
server <- function(input, output, session) {
	...
	# Input
	############################################################################
	observeFakeLocalFileInput(input, session, 'sampleAnnotFile', '/data/samplesheet.csv')
